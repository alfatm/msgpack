...
func RegisterExt(id int8, value interface{}) {
	...
	if _, ok := extTypes[id]; ok {
		panic(zerror.NewError("msgpack: ext with id=%d is already registered", id))
	}
...
func makeExtDecoder(typeID int8, dec decoderFunc) decoderFunc {
	return func(d *Decoder, v reflect.Value) error {
		...
		if id != typeID {
			return zerror.NewError("msgpack: got ext type=%d, wanted %d", id, typeID)
		}
...
func (d *Decoder) parseExtLen(c codes.Code) (int, error) {
	...
	case codes.Ext32:
		...
		return int(n), err
	default:
		return 0, zerror.NewError("msgpack: invalid code=%x decoding ext length", c)
	}
}
...
func (d *Decoder) extInterface(c codes.Code) (interface{}, error) {
	...
	if !ok {
		return nil, zerror.NewError("msgpack: unknown ext id=%d", extID)
	}
